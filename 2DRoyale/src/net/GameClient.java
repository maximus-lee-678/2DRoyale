...
public class GameClient extends Thread {
	...
	private void readPacket(byte[] data, InetAddress address, int port) {
		...
		case 1:
			// LOGIN
			System.out.println("[" + address.getHostAddress() + ":" + port + "] " + msgData + " has joined the game...");
		...
		case 2:
			...
			game.getPlayers().remove(playerIndex(msgData));	//delete new player to playerList. Got index to delete from playerIndex function
			break;
		case 3:
			// MOVEMENT
		...
		case 4:
			...
			game.getPlayers().get(playerIndex(dataArr[0])).updateDirection(mouseX, mouseY);	//get player and call move() to update player world coordinates
			break;
		default:
		case 0:
			break;
	...
	public void sendData(byte[] data) {						//send data from client to server
		DatagramPacket packet = new DatagramPacket(data, data.length, ipAddress, 2207);	//2207 is a random port number i thought of
		try {
			socket.send(packet);
		} catch (IOException e) {
			e.printStackTrace();
